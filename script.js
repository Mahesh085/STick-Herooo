    ...
    function a(a) {
        ...
        return this.init = function() {
            ...
            this.$game = $("#game").css({
                ...
                width: k + "px"
            }), this.$qrCode = $(".qr-code"), H && this.$qrCode.hide(), this.$gametitle = $(".game-title"), this.$gameover = $(".game-over"), this.$welcome = $(".welcome"), this.$heropick = $(".heropick"), this.$draw = $(".draw"), this.$drawTotal = $(".draw-total"), this.$drawIcon = $(".btn-draw .icon"), this.$drawPlate = $(".draw-plate"), this.$drawResult = $(".draw-result"), this.$drawPrize = $(".draw-prize"), this.$newHeroIcon = $(".btn-hero .new"), this.$share = $(".share"), this.$livescore = $(".live-score"), this.$watermelon = $(".watermelon"), this.$instruction = $(".instruction"), this.$about = $(".about"), this.$perfect = $(".perfect"), this.$score = $(".score"), this.$best = $(".best"), this.$total = $(".total"), this.$movedStick = $("nothing"), this._currentState = K.WELCOME, this.total = parseInt(store("total") || 0, 10), this.$total.text(this.total), this.isNew = store("stick-hero-404") + "" == "true", this.heroInit(), this.switchHero(this.hero), this.drawInit()
        }, this.heroInit = function() {
            this.hero = store("hero") || 1, 6 === this.hero && (this.hero = 1), this.$heros = $(".hero-p");
            ...
            this.drawTotal = store("drawTotal"), this.drawTotal = null === this.drawTotal ? x : parseInt(this.drawTotal, 10), this.updateDraw()
            ...
            $(".btn-play").on(I, function() {
                ...
                a.$heropick.toggleClass("in"), a.$draw.removeClass("in"), this.isNew || (store("stick-hero-404", !0), a.$newHeroIcon.hide()), b.stopPropagation(), $(document).off(I, ".overlay"), $(document).on(I, ".overlay", function() {
                    $(document).off(I, ".overlay"), a.$draw.removeClass("in"), a.$heropick.removeClass("in")
                })
            }), $(".btn-draw").on(I, function(b) {
                a.$draw.toggleClass("in"), a.$heropick.removeClass("in"), b.stopPropagation(), $(document).off(I, ".overlay"), $(document).on(I, ".overlay", function() {
                    $(document).off(I, ".overlay"), a.$heropick.removeClass("in"), a.$draw.removeClass("in")
                })
            ...
            this.score = 0, this.count = 0, this.isDrawing = !1, this.best = store("best") || 0, this.$heroContainer = this.$hero.parent(), this.$game.removeClass("bounce bg1 bg2 bg3 bg4 bg5 bg6").addClass("bg" + this._getRandom(1, 6)), this.$gametitle.removeClass("hinge"), this.$livescore.hide(), this.$gameover.hide(), this.$welcome.hide(), this.updateScore(), $(".box, .stick").remove(), this.BOX1 = {
                left: 0,
                width: o
            ...
            this.$gameover.hide(), this.$livescore.hide(), this.$watermelon.hide(), this.$heropick.show(), this.$draw.show(), this.$welcome.show()
        }, this.preBegin = function() {
            this.$welcome.hide(), this.$gameover.hide(), this.$heropick.hide(), this.$draw.hide(), this.$livescore.show(), this.$watermelon.show(), this.$instruction.addClass("in"), this.BOX2 = this._createBox(), this.$box2 = $("<div />").addClass("box").css({
                height: p + "px",
            ...
            if (!(this.isDrawing || this.drawTotal <= 0)) {
                ...
                }), this.isDrawing = !0, this.updateDraw(-1), this.$drawResult.removeClass("in"), this.$drawPlate.addClass("start").css({
            ...
            this.$drawPrize.text(b), this.$drawResult.addClass("in"), this.$drawPlate.removeClass("start").css({
                "-webkit-transform": "rotate(" + -a + "deg)",
                transform: "rotate(" + -a + "deg)"
            })
        }, this.getPrize = function(a) {
            for (var b, c = 0; c < M.length; c++)
                if (M[c].from <= a && a < M[c].from + 360 * M[c].per / 100) return "number" == typeof M[c].prize ? (this.updateTotal(M[c].prize), b = M[c].prize + " Watermelon!") : (this.unlockHero(N), b = "Super Hero！！")
            ...
            void 0 !== a && (this.drawTotal += a), store("drawTotal", this.drawTotal), this.$drawTotal.text(this.drawTotal), this.drawTotal > 0 ? this.$drawIcon.show() : this.$drawIcon.hide()
